...
def func():
    ...
    if nid not in nic_metric_deltas:
        ...
        nic_out_metrics[nid] = {"aries_id": "", "dt": 0, "df": 0, "ds": 0, "s2f": 0, "dpif": 0, "dprocs": 0, "n2p": 0,
                                "hpg": 0}
        nic_ts_prev[nid] = 0
        curr_time = tcurr
    else:
        if tcurr > nic_ts_prev[nid]:
            ...
            # df/dt
            nic_out_metrics[nid]["ds"] = safe_div(nic_metric_deltas[nid]["AR_NIC_NETMON_ORB_EVENT_CNTR_REQ_STALLED"],
                                                  dt)
            # ds/dt
            nic_out_metrics[nid]["df"] = safe_div(nic_metric_deltas[nid]["AR_NIC_NETMON_ORB_EVENT_CNTR_REQ_FLITS"], dt)
            #  s2f
            nic_out_metrics[nid]["s2f"] = safe_div(nic_metric_deltas[nid]["AR_NIC_NETMON_ORB_EVENT_CNTR_REQ_STALLED"],
                                                   nic_metric_deltas[nid]["AR_NIC_NETMON_ORB_EVENT_CNTR_REQ_FLITS"])
            # dprocs/dt
            nic_out_metrics[nid]["dprocs"] = safe_div(
                nic_metric_deltas[nid]["AR_NIC_RSPMON_PARB_EVENT_CNTR_PI_STALLED"], dt)
            # dpif/dt
            nic_out_metrics[nid]["dpif"] = safe_div(nic_metric_deltas[nid]["AR_NIC_RSPMON_PARB_EVENT_CNTR_PI_FLITS"],
                                                    dt)
            #  n2p
            nic_out_metrics[nid]["n2p"] = safe_div(nic_metric_deltas[nid]["AR_NIC_RSPMON_PARB_EVENT_CNTR_PI_STALLED"],
                                                   nic_metric_deltas[nid]["AR_NIC_RSPMON_PARB_EVENT_CNTR_PI_FLITS"])
            #  hpg
            nic_out_metrics[nid]["hpg"] = safe_div(nic_metric_deltas[nid]["AR_NIC_RSPMON_PARB_EVENT_CNTR_IOMMU_PKTS"],
                                                   nic_metric_deltas[nid]["AR_NIC_RSPMON_PARB_EVENT_CNTR_PI_PKTS"])
    ...
    if curr_time < tcurr:
        ...
        print("time".ljust(8), "nid".ljust(3), "aries_id".ljust(10), "dt", "df", "ds", "s2f", "dpif", "dprocs", "n2p",
              "hpg")
        if len(nodes) == 0:
